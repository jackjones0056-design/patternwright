...
Patternwright OS → session login → `GET/PUT /api/state` → shared persistent state
...
Auth → `POST /api/auth/login|logout`, `GET /api/auth/me|csrf` → HttpOnly session + CSRF
...
- Production deploys use session-based admin login (password from env, scrypt-verified). Optional admin API key remains for automation.
- Public Fit Check stays public but rate-limited; OS/mutating APIs require auth when configured.
