...
$('#settings-form').addEventListener('submit',async e=>{e.preventDefault();state.settings=Object.fromEntries(new FormData(e.target).entries());save();toast('Settings saved');await hydrateRemoteState()});function fillSettings(){const f=$('#settings-form');Object.entries(state.settings||{}).forEach(([k,v])=>{if(f.elements[k])f.elements[k].value=v});$('#storage-status').textContent=HAS_STORAGE?'Browser fallback storage is available.':'Restricted preview detected: browser persistence is unavailable; use the backend or export before closing.';if(backendAvailable)setBackendStatus(true)}
...
if(loginForm)loginForm.addEventListener('submit',async e=>{e.preventDefault();const fd=new FormData(loginForm);const err=document.querySelector('#login-error');if(err)err.textContent='';try{await login(String(fd.get('username')||'').trim(),String(fd.get('password')||''));toast('Signed in');await hydrateRemoteState()}catch(ex){if(err)err.textContent=ex.message||'Sign-in failed';toast('Sign-in failed')}});
const logoutBtn=document.querySelector('#logout-btn');
if(logoutBtn)logoutBtn.addEventListener('click',async()=>{await logout();remoteHydrated=false;setBackendStatus(false,'Signed out');toast('Signed out')});
